...
LP Forge for ClickMax is a React + TypeScript web app that generates landing page HTML code using Google Gemini AI. Users input their sales copy, visual references, and instructions, and receive production-ready HTML optimized for the ClickMax page builder.
...
No test or lint scripts are configured in this project yet.

## Application Architecture

### Application Flow
1. User fills form with copy, visual references, and instructions (`InputForm.tsx`)
2. Form submits to `generateLandingPageHtml()` in `services/geminiService.ts`
3. Gemini AI generates responsive HTML with embedded CSS
4. Result displayed in preview iframe or code view (`ResultViewer.tsx`)
5. User copies HTML for use in ClickMax

### Key Files
- `App.tsx` - Main component, state management, view switching (FORM/PREVIEW/CODE)
- `services/geminiService.ts` - Gemini API integration, prompt engineering
- `components/InputForm.tsx` - Three-textarea input form
- `components/ResultViewer.tsx` - Preview iframe + code viewer with copy button
- `types.ts` - TypeScript interfaces (`LandingPageData`, `GenerationResult`, `ViewMode`)

### Environment
- Requires `GEMINI_API_KEY` in `.env.local`
- Uses Vite with React plugin, serves on port 3000
